...
int main( int argc, char** argv )
{
  ...
  cv::Mat_<cv::Vec3b> destination_anaglyph ( source.rows, source.cols/2 );
  ...
  // Anaglyph:
  cout << "Width : " << source.size().width << endl;
  cout << "Height: " << source.size().height << endl;
	 ...
	 #pragma omp parallel for
      for (int i=0;i<source.rows;i++)
	      for (int j=0;j<source.cols/2;j++)
        {
            ...
            destination_anaglyph(i,j)[0] = 0.249*r2+0.587*g2+0.114*b2; // b
            destination_anaglyph(i,j)[1] = 0; // g
            destination_anaglyph(i,j)[2] = 0.249*r1+0.587*g1+0.114*b1; // r
        }
    }
  ...
  cv::imshow("Anaglyph Image", destination_anaglyph );
