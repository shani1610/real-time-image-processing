...
__global__ void process(const cv::cuda::PtrStep<uchar3> src, cv::cuda::PtrStep<uchar3> dst, int rows, int cols, const int kernel_size_div2, double sigma )
{
...
void startCUDA ( cv::cuda::GpuMat& src, cv::cuda::GpuMat& dst, const int kernel_size_div2, double sigma  )
{
